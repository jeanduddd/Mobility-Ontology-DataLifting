...
*   **[GraphQuery/](./GraphQuery)** : Contains a Jupyter notebook used to query and explore the knowledge graph produced by the data lifting pipeline. The notebook provides structural and competency questions.
...
Docker is used to containerize our semantic lifting engine, **Morph-xR2RML**, as well as the local **Virtuoso** triple store used to store and query the generated knowledge graph. This allows us to run the complete data conversion and knowledge graph environment in isolated containers, meaning you don't need to install Java, Virtuoso, or configure their dependencies directly on your device.
...
**About the Triple Store:**
[OpenLink Virtuoso](https://github.com/openlink/virtuoso-opensource) is used as the local RDF triple store for storing and querying the knowledge graph generated by the lifting engine.

To integrate the tool, the following steps were performed during development:

* The Virtuoso Docker image was integrated directly into our project environment.
* A local Virtuoso instance was configured to store the RDF triples generated by the Morph-xR2RML lifting pipeline in the `DataLifting_output` folder.
* The configuration was adapted to expose the Virtuoso SPARQL endpoint locally and allow the generated knowledge graph to be queried by the [GraphQuery](./GraphQuery/) notebook.

It means that there is no extra manual installation step. Virtuoso is already integrated and containerized within this repository. Docker will automatically build and run the local triple store when you launch the tool, making the generated knowledge graph available for SPARQL queries.
...
To query the generated knowledge graph, launch the [notebook](./GraphQuery/graphQuerying.ipynb) located in the [GraphQuery](./GraphQuery/) directory.
The notebook was developed and executed in Visual Studio Code using the Jupyter extension and a Python environment.

The first cells of the notebook automatically import the data generated by the pipeline from the DataLifting_output folder into the local Virtuoso triple store.

Once the data has been loaded, write your SPARQL query as a string and pass it to the `executeSparqlQuery()` function:
...
The function returns the query results as a Pandas DataFrame. To display the results, simply use:
